...
impl Select {
    ...
    ///
    /// The returned [`InnerResponse::inner`] is `Some(value)` when the user
    /// picked a different option this frame, `None` otherwise.
    /// [`InnerResponse::response`] is the trigger's response.
    pub fn show(&mut self, ui: &mut egui::Ui) -> InnerResponse<Option<String>> {
        ...
        if is_open {
            ...
            let scroll_h = (item_h * max_visible.min(self.options.len()) as f32) + inner_padf * 2.0;
        ...
        InnerResponse::new(changed, trigger_resp)
    }
}
